      ...
      <div className="fixed right-0 top-0 bottom-0 w-full max-w-[420px] bg-white z-50 flex flex-col shadow-2xl animate-slide-in">
          ...
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-8">
              ...
              <Link
                to="/shop"
                onClick={() => toggleCart(false)}
                className="btn-outline text-stone-600 border-stone-300 hover:bg-stone-900 hover:text-white hover:border-stone-900"
              >
                Continue Shopping
              </Link>
            </div>
              ...
              {items.map((item) => (
                <div key={`${item.product.id}-${item.size}-${item.color}`} className="flex gap-4 py-5 first:pt-2">
                  <Link
                    ...
                    onClick={() => toggleCart(false)}
                    className="w-[88px] h-[110px] flex-shrink-0 bg-stone-50 overflow-hidden group"
                  >
                    <img
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-between">
                    <div>
                      <Link
                        to={`/product/${item.product.id}`}
                        onClick={() => toggleCart(false)}
                        className="text-sm font-medium text-stone-900 truncate block hover:text-stone-600 transition-colors"
                      >
                        {item.product.name}
                      </Link>
                    ...
                    <button
                      ...
                      className="text-[10px] tracking-[0.15em] uppercase text-stone-400 hover:text-stone-900 transition-colors mt-1 self-start"
                    >
                      Remove
                    </button>
                  </div>
        ...
        {items.length > 0 && (
          <div className="border-t border-stone-100 px-6 py-6 bg-stone-50/50">
            ...
            <button
              onClick={() => toggleCart(false)}
              className="w-full mt-3 py-2.5 text-[11px] tracking-[0.12em] uppercase text-stone-400 hover:text-stone-700 transition-colors font-medium"
            >
              Continue Shopping
            </button>
          </div>
