...
export default function AIChatBot() {
  ...
  const handleQuickAction = (action: string) => {
    const message =
      QUICK_ACTION_MESSAGES[
        action as keyof typeof QUICK_ACTION_MESSAGES
      ];
  ...
  const clearHistory = () => {
    if (
      confirm('Clear chat history? This cannot be undone.')
    ) {
      setMessages([
        {
          id: '0',
  ...
  const messageBotBgClass = isDark
    ? 'bg-stone-800'
    : 'bg-stone-100';
  ...
  return (
    <div
      ref={chatRef}
      className={`fixed bottom-6 right-6 z-50 w-full max-w-sm h-[600px] ${bgClass} rounded-2xl shadow-2xl flex flex-col border border-stone-200 ${
        isDark ? 'border-stone-700' : ''
    ...
    >
      ...
      {!isMinimized && (
        <>
          {/* Messages */}
          <div
            className={`flex-1 overflow-y-auto p-5 space-y-4 ${bgClass}`}
          >
            {messages.map((msg) => (
              <div
                key={msg.id}
                ...
                <div
                  className={`max-w-[70%] px-4 py-2.5 rounded-xl text-[13px] leading-relaxed ${
                    msg.role === 'user'
                      ...
                      : `${messageBotBgClass} ${textClass} rounded-bl-sm border border-stone-300 ${
                          isDark
                            ? 'border-stone-600'
                            : ''
                        }`
            ...
            {loading && (
              <div className="flex gap-2.5 justify-start animate-fade-in">
                ...
                <div
                  className={`px-4 py-2.5 rounded-xl rounded-bl-sm border ${messageBotBgClass} ${
                    isDark
                      ? 'border-stone-600'
                      : 'border-stone-300'
                  }`}
          ...
          {messages.length <= 2 && !loading && (
            <div
              className={`px-5 pb-3 grid grid-cols-2 gap-2 flex-shrink-0 border-t ${
                isDark
                  ? 'border-stone-700'
                  : 'border-stone-200'
              }`}
          ...
          {/* Footer */}
          <div
            className={`px-5 py-2 flex justify-end border-t text-[11px] flex-shrink-0 ${
              isDark
